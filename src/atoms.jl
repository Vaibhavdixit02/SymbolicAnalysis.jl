...
add_dcprule(quad_form, (array_domain(ℝ,1), semidefinite_domain()), Positive, Vex, (increasing_if_positive, Increasing))
...
add_dcprule(log, array_domain(ℝ, 2), Positive, Cave, Increasing)

@register_symbolic LinearAlgebra.inv(A::Symbolics.Arr)

add_dcprule(inv, semidefinite_domain(), AnySign, Vex, Decreasing)

@register_symbolic LinearAlgebra.sqrt(A::Symbolics.Arr)

add_dcprule(sqrt, semidefinite_domain(), Positive, Cave, Increasing)
